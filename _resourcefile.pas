...
// *****************************************************************************
// R16 file definitions
// *****************************************************************************

const R16FILE_DATA = 0;
const R16FILE_UI = 1;
const R16FILE_MOUSE = 2;

const R16_FILENAMES: array[0..2] of string = ('DATA.R16', 'UI_ENG.R16', 'MOUSE.R16');
...
type
  ...
  public
    ...
    // R16 related data
    r16_file_contents: array of byte;
    r16_modified: boolean;
    r16_file_entry_positions: array of integer;
    r16_file_entry_count: integer;
  ...
  public
    // General procedures
    procedure init(filenum: integer);

    // DATA.R16 related procedures
    procedure load_r16(force: boolean);
    procedure save_r16;
    procedure process_r16_entries;
    ...
    procedure resize_r16_contents(position, old_size, new_size: integer);
...
var
  ResourceFile: array[0..Length(R16_FILENAMES)-1] of TResourceFile;

implementation

uses Forms, SysUtils, IniFiles, Classes, _settings, _dispatcher, _missionini, _colours, Math, pngimage;

procedure TResourceFile.init(filenum: integer);
begin
...
procedure TResourceFile.load_r16(force: boolean);
var
  tmp_filename: String;
  r16_file: file of byte;
  r16_file_size: integer;
begin
  tmp_filename := find_file('Data\' + R16_FILENAMES[my_filenum], 'graphics');
  if (tmp_filename = '') or ((tmp_filename = r16_filename) and not force) then
    exit;
  r16_filename := tmp_filename;

  // Clear all previously loaded data
  clear_structure_image_data;
  SetLength(r16_file_contents, 0);
  // Load file into buffer
  AssignFile(r16_file, tmp_filename);
  Reset(r16_file);
  r16_file_size := FileSize(r16_file);
  SetLength(r16_file_contents, r16_file_size);
  BlockRead(r16_file, r16_file_contents[0], r16_file_size);
  CloseFile(r16_file);
  r16_modified := false;
  // Parse file
  process_r16_entries;
...
procedure TResourceFile.save_r16;
begin
  if (r16_filename = '') or not r16_modified then
    exit;
  if not manage_filesave(r16_filename, 'Data\' + R16_FILENAMES[my_filenum], evStructuresFilenameChange) then
    exit;
  save_binary_file(r16_filename, r16_file_contents[0], Length(r16_file_contents));
  r16_modified := false;
end;

procedure TResourceFile.process_r16_entries;
var
  r16_file_size: integer;
  index: integer;
  position: integer;
...
begin
  index := 0;
  position := 0;
  r16_file_size := Length(r16_file_contents);
  while position < r16_file_size do
  begin
    if (my_filenum = R16FILE_UI) and (index mod 3 = 0) and (index < 45) then
      inc(position, 8);
    first_byte := r16_file_contents[position];
    // Reallocate r16_file_entry_positions capacity if needed
    if index = Length(r16_file_entry_positions) then
      SetLength(r16_file_entry_positions, Length(r16_file_entry_positions) + DATA_R16_FILE_ENTRY_POSITIONS_ALLOC_STEP);
    r16_file_entry_positions[index] := position;
    ...
    header := Addr(r16_file_contents[position]);
  ...
  r16_file_entry_count := index;
  // Initialize entry mapping
  SetLength(structure_image_entry_mapping, r16_file_entry_count);
  for index := 0 to r16_file_entry_count - 1 do
  begin
...
procedure TResourceFile.clear_structure_image_data;
var
  i: integer;
begin
  // Clean entry position map
  SetLength(r16_file_entry_positions, 0);
  r16_file_entry_count := 0;
...
procedure TResourceFile.clear_last_structure_image(entry_index: integer; is_stealth: boolean);
begin
...
procedure TResourceFile.load_structure_image(entry_index, house_index: integer; is_unit, is_stealth: boolean);
var
...
begin
  ...
  if r16_file_contents[r16_file_entry_positions[entry_index]] = 0 then
  begin
    // Empty entry
    if not is_stealth then
  ...
  header := Addr(r16_file_contents[r16_file_entry_positions[entry_index]]);
  image_data_8bpp := Addr(r16_file_contents[r16_file_entry_positions[entry_index] + sizeof(TR16EntryHeader)]);
  image_data_16bpp := Addr(r16_file_contents[r16_file_entry_positions[entry_index] + sizeof(TR16EntryHeader)]);
    ...
    begin
      ...
      begin
        ...
        if is_stealth and (pixel <> 0) then
          pixel := 248;
        // House colox pixel
        if (pixel >= 240) and (house_index >= 0) then
        begin
          dec(pixel, 240);
...
procedure TResourceFile.recolor_structure_image(image_index, house_index: integer);
var
  i: integer;
begin
...
function TResourceFile.get_structure_image(entry_index, house_index: integer; is_unit, is_stealth: boolean; var was_already_loaded: boolean): TStructureImagePtr;
var
  image_index: integer;
begin
  was_already_loaded := false;
  result := nil;
  if r16_filename = '' then
    exit;
  if entry_index >= r16_file_entry_count then
    exit;
...
function TResourceFile.get_structure_image_header(entry_index: integer): TR16EntryHeaderPtr;
begin
  if entry_index < r16_file_entry_count then
    result := Addr(r16_file_contents[r16_file_entry_positions[entry_index]])
  else
    result := nil;
end;

function TResourceFile.get_structure_image_palette(entry_index: integer): TR16PalettePtr;
var
...
begin
  ...
  while r16_file_contents[r16_file_entry_positions[palette_origin_index]] <> 1 do
    dec(palette_origin_index);
  palette_origin_header := Addr(r16_file_contents[r16_file_entry_positions[palette_origin_index]]);
  result := Addr(r16_file_contents[r16_file_entry_positions[palette_origin_index] + sizeof(TR16EntryHeader) + palette_origin_header.ImageWidth * palette_origin_header.ImageHeight]);
end;

function TResourceFile.get_raw_structure_image(entry_index: integer): TBitmap;
var
  header: TR16EntryHeaderPtr;
  image_data_8bpp: TByteArrayPtr;
  image_data_16bpp: TWordArrayPtr;
...
begin
  ...
  begin
    ...
    for i := 0 to 255 do
    begin
      colour := Colours.colour_16bto32b(palette.colors[i]);
...
procedure TResourceFile.set_default_palette_colors(entry_index: integer);
var
  palette: TR16PalettePtr;
  i: integer;
begin
...
function TResourceFile.remap_image_colors(entry_index: integer; ini_filename: String): boolean;
var
  ini: TMemIniFile;
  strings: TStringList;
...
procedure TResourceFile.invalidate_colors;
var
  i: integer;
begin
  for i := 0 to structure_images_count - 1 do
...
function TResourceFile.get_image_entry_position(entry_index: integer): integer;
begin
  if (entry_index < r16_file_entry_count) and (entry_index >= 0) then
    result := r16_file_entry_positions[entry_index]
  else if entry_index = r16_file_entry_count then
    result := Length(r16_file_contents)
  else
    result := -1;
end;

function TResourceFile.get_image_entries_size(first_entry_index, num_entries: integer): integer;
var
  i: integer;
begin
  result := 0;
  for i := first_entry_index to (first_entry_index + num_entries - 1) do
    if i < r16_file_entry_count then
      inc(result, get_image_entry_position(i+1) - get_image_entry_position(i))
    else
      break;
end;

procedure TResourceFile.resize_r16_contents(position, old_size, new_size: integer);
var
  old_total_size: integer;
begin
  if new_size = old_size then
    exit;
  old_total_size := Length(r16_file_contents);
  if new_size > old_size then
  begin
    SetLength(r16_file_contents, old_total_size + new_size - old_size);
    Move(r16_file_contents[position + old_size], r16_file_contents[position + new_size], old_total_size - position - old_size);
  end else
  begin
    Move(r16_file_contents[position + old_size], r16_file_contents[position + new_size], old_total_size - position - old_size);
    SetLength(r16_file_contents, old_total_size + new_size - old_size);
...
procedure TResourceFile.modify_image_data(entry_index: integer; var data; data_size: integer);
var
  entry_size: integer;
...
begin
  ...
  resize_r16_contents(entry_position, entry_size, data_size);
  Move(data, r16_file_contents[entry_position], data_size);
...
procedure TResourceFile.add_empty_image_entries(entry_index, num_entries: integer);
var
  entry_position: integer;
begin
  if num_entries = 0 then
    exit;
  entry_position := get_image_entry_position(entry_index);
  resize_r16_contents(entry_position, 0, num_entries);
  FillChar(r16_file_contents[entry_position], num_entries, 0);
  // Invalidate all cached data and re-process them again
  clear_structure_image_data;
  process_r16_entries;
  r16_modified := true;
...
procedure TResourceFile.remove_image_entries(entry_index, num_entries: integer);
var
...
begin
  ...
  entry_position := get_image_entry_position(entry_index);
  resize_r16_contents(entry_position, entries_size, 0);
  // Invalidate all cached data and re-process them again
  clear_structure_image_data;
  process_r16_entries;
  r16_modified := true;
...
procedure TResourceFile.change_image_entry_count(first_entry_index, old_num_entries, new_num_entries: integer);
begin
...
procedure TResourceFile.swap_image_entries(src_entry_index, dest_entry_index, src_num_entries, dest_num_entries: integer);
var
...
begin
  ...
  begin
    ...
    Move(r16_file_contents[dest_entry_position], tmp_data[0], dest_entries_size);
    Move(r16_file_contents[src_entry_position], r16_file_contents[dest_entry_position], src_entries_size);
    Move(tmp_data[0], r16_file_contents[dest_entry_position + src_entries_size], dest_entries_size);
  end else
  begin
    SetLength(tmp_data, src_entries_size);
    Move(r16_file_contents[src_entry_position], tmp_data[0], src_entries_size);
    Move(r16_file_contents[dest_entry_position], r16_file_contents[src_entry_position], dest_entries_size);
    Move(tmp_data[0], r16_file_contents[src_entry_position + dest_entries_size], src_entries_size);
  end;
...
procedure TResourceFile.export_image_entries(filename: string; entry_index, num_entries: integer);
var
  entries_size: integer;
  entry_position: integer;
begin
  ...
  entries_size := get_image_entries_size(entry_index, num_entries);
  entry_position := get_image_entry_position(entry_index);
  save_binary_file(filename, r16_file_contents[entry_position], entries_size);
end;

function TResourceFile.import_image_entries(filename: string; entry_index, num_entries: integer): boolean;
var
  entries_size: integer;
  entry_position: integer;
...
begin
  ...
  entries_size := get_image_entries_size(entry_index, num_entries);
  entry_position := get_image_entry_position(entry_index);
  resize_r16_contents(entry_position, entries_size, size);
  Move(buffer[0], r16_file_contents[entry_position], size);
  SetLength(buffer, 0);
  // Invalidate all cached data and re-process them again
  clear_structure_image_data;
  process_r16_entries;
...
procedure TResourceFile.export_single_image(filename: string; entry_index: integer);
var
  tmp_bitmap: TBitmap;
  PNG: TPNGObject;
begin
...
procedure TResourceFile.erase_single_image(entry_index: integer);
var
  zero_byte: byte;
begin
  zero_byte := 0;
...
function TResourceFile.import_single_image(filename: string; entry_index: integer; is_icon: boolean): boolean;
var
  tmp_bitmap: TBitmap;
  PNG: TPNGObject;
  buffer: array of byte;
